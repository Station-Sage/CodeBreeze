...
const PATTERNS: PatternDef[] = [
  ...
  {
    regex: /^(.+?\.[ch](?:pp|xx)?):(\d+):(\d+):\s+(error|warning|fatal error):\s+(.+)$/gm,
    fileIdx: 1,
    lineIdx: 2,
    sevIdx: 4,
    msgIdx: 5,
  ...
  {
    regex: /^(.+?\.(?:java|kt|kts)):(\d+):\s+(error|warning):\s+(.+)$/gm,
    fileIdx: 1,
    lineIdx: 2,
    sevIdx: 3,
    msgIdx: 4,
  ...
  {
    regex: /^(.+?\.swift):(\d+):(\d+):\s+(error|warning):\s+(.+)$/gm,
    fileIdx: 1,
    lineIdx: 2,
    sevIdx: 4,
    msgIdx: 5,
...
export function parseErrorOutput(output: string): ParsedErrorInfo[] {
  ...
  for (const { regex, fileIdx, lineIdx, sevIdx, msgIdx } of PATTERNS) {
    ...
    while ((match = regex.exec(output)) !== null) {
      ...
      const severity: 'error' | 'warning' = (sevRaw || 'error').includes('warn')
        ? 'warning'
        : 'error';
